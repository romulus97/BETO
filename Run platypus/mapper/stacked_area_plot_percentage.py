...
Energy_avg = np.zeros((len(df_ha),1))
...
for year in years:
    ...
    for s in solution:
        ...
        Energy_total_MJ_avg = sum(Energy_total_MJ[s])/len(Energy_total_MJ[s])
        Energy_avg[s] = Energy_total_MJ_avg    
...
CS_p = CS_total_MJ*100/Energy_total_MJ
G_p = G_energy_total_MJ*100/Energy_total_MJ
A_p = A_energy_total_MJ*100/Energy_total_MJ

CS_avg = CS_avg*100/Energy_avg
G_avg = G_energy_avg*100/Energy_avg
A_avg = A_energy_avg*100/Energy_avg
...
plt.axvline(x=(int(n)),color='black',linewidth=4)
