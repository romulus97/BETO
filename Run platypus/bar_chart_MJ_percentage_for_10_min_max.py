...
# MIN SOLUTION 
n = 1
min_10_sol = sorting.tail(n)
ind_min = list(min_10_sol.index.values)
...
for ind in ind_min:
    ...
    CS_p = CS_total_MJ[ind]*100/Energy_total_MJ[ind]
    G_p = G_energy_total_MJ[ind]*100/Energy_total_MJ[ind]
    A_p = A_energy_total_MJ[ind]*100/Energy_total_MJ[ind]
    ...
    index = [str (b) for b in ind_min]
    

    #ax.set_title(index[a] +' Min energy shortfall solution')
    ax.set_title(index[a] +' Min GHG emission solution')
    #ax.set_title(index[a] +' Min cost solution as percentage')
    ...
    #fig.savefig(index[a] +' MJ_bar_chart_min_energy_shortfall_p.png',bbox_inches='tight', dpi=300)
    fig.savefig(index[a] +' MJ_bar_chart_min_GHG_emission_p.png',bbox_inches='tight', dpi=300)
    #fig.savefig(index[a] +' MJ_bar_chart_min_cost_p.png',bbox_inches='tight', dpi=300)    
...
# MIN SOLUTION 
n = 1
max_10_sol = sorting.head(n)
ind_max = list(max_10_sol.index.values)


for indm in ind_max:
    am = ind_max.index(indm)


    CS_pm = CS_total_MJ[indm]*100/Energy_total_MJ[indm]
    G_pm = G_energy_total_MJ[indm]*100/Energy_total_MJ[indm]
    A_pm = A_energy_total_MJ[indm]*100/Energy_total_MJ[indm]
    ...
    # ax.bar(labels, G_p_t, width,  bottom = CS_p_t, label='Grass percentage of MJ/kg', color = ['#ffba08'])
    # ax.bar(labels, A_p_t, width,  bottom = G_p_t, label='Algae percentage of MJ/kg', color = ['#d00000'])
    abc = pd.DataFrame(zip(CS_pm_t,G_pm_t,A_pm_t), index=labels)
    abc = abc.apply(lambda x: round(x,1))
    abc.plot(kind='bar', stacked=True, ax=ax, color = ['#2b9348','#ffba08','#d00000'])
    ax.legend('',frameon=False)
    ax.set_ylabel('percentage', fontsize=10)
    ...
    index = [str (b) for b in ind_max]
    

    #ax.set_title(index[a] +' Max energy shortfall solution')
    ax.set_title(index[am] +' Max GHG emission solution')
    #ax.set_title(index[a] +' Max cost solution as percentage')
    ...
    #fig.savefig(index[am] +' MJ_bar_chart_max_energy_shortfall_p.png',bbox_inches='tight', dpi=300)
    fig.savefig(index[am] +' MJ_bar_chart_max_GHG_emission_p.png',bbox_inches='tight', dpi=300)
    #fig.savefig(index[am] +' MJ_bar_chart_max_cost_p.png',bbox_inches='tight', dpi=300)    
