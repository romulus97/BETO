...
# reduced_counties = []
df_reduced = pd.read_csv('reduced.csv',header=0)
reduced_counties = list(df_reduced['Reduced'])
...
# df_reduced = pd.DataFrame(reduced_counties)
# df_reduced.columns = ['Reduced']
...
for h in hubs:
    ...
    # locations = []
    locations = range(1,num_refineries+1)
    ...
    for i in range(0,len(LC)):
        ...
        # CS_farm_kg += CS_per_ha*V[i]
    
        # Capital costs (need to expand)
        CS_cultivation_capex += V[i]*LC[i]
        
        # Operating costs (need to expand)
        harvesting = 38.31*ha_to_acre # $ per ha
        CS_cultivation_opex += V[i]*(seeds_per_ha*.00185 + fertilization_per_ha[0]*0.55 + fertilization_per_ha[1]*0.46 + fertilization_per_ha[2]*0.50 + lime_per_ha*0.01 + harvesting) #herbicide in per ha??
        
        # Automatic flow to pre-processing hub
        CS_C2H_prod[i,:] = C2H_map[i,:]*CS_per_ha*V[i]
    
        CS_cultivation_opex += CS_per_ha*V[i]*(lb_to_kg)*(1/1500)*0.50*C2H[i]
        
        # Cultivation constraints (land limits)
        Constraints.append(V[i] - LL[i])  
    ...
    # Constraints.append(sum(CS_farm_kg) - 12100000)
    
    Constraints = list(Constraints)
    
    print ([CS_refinery_capex/sum(CS_ethanol), CS_travel_opex/sum(CS_ethanol), sum(CS_ethanol)])
...
# num_variables = len(reduced_land_costs) + len(hubs) * len(locations)
# num_constraints = len(reduced_land_costs) + len(hubs) + 2
# num_objs = 2

# problem = Problem(num_variables,num_objs,num_constraints)
# problem.types[0,len(reduced_land_costs)] = Real(0,max(reduced_land_limits))
# problem.types[len(reduced_land_costs):] = Real(0,UB)
