...
def simulate(
    ...
    CG_cultivation_proc = 0 #make them vectors of zeros
    SB_cultivation_proc = 0
    G_cultivation_proc = 0
    A_cultivation_proc = 0
    ...
    CG_cultivation_proc += np.sum((CG_prod_total*(Proc_corn_cost))) 
    SB_cultivation_proc += np.sum((SB_prod_total*(Proc_soy_cost)))
    G_cultivation_proc += np.sum((G_prod_total*(Proc_grass_cost)))
    A_cultivation_proc += np.sum((A_prod_total*(Proc_algae_cost)))

    Constraints = list(Constraints)
    
        
    # Returns list of objectives, Constraints
    biomass_cost_corn = CG_cultivation_capex + CG_cultivation_opex + CG_cultivation_proc
    biomass_cost_soy = SB_cultivation_capex + SB_cultivation_opex + SB_cultivation_proc
    biomass_cost_grass = G_cultivation_capex + G_cultivation_opex + G_cultivation_proc
    biomass_cost_algal = A_cultivation_capex + A_cultivation_opex + A_cultivation_proc
