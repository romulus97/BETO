# -*- coding: utf-8 -*-
"""
Created on Mon Jun 20 17:08:52 2022
...
CG_kg_tot_den = np.zeros((len(LC),len(years)))
...
for ind in ind_min_cost:
    ...
    for year in years:
        ...
        CG_prod = C_ha[:,ind]*Y           # total corn biomass production for a specific min cost solution for a year (kg)
        CG_kg_tot[:,i] = CG_prod
        #CG_prod_10[:,a] = CG_prod         # total corn biomass production for 10 min cost solutions for a year (kg)
    ...
    fig1 = px.scatter(df_yield_corn, x = 'land_cost' , y = 'land_limits' , color = 'State', size = 'corn kg')
    fig1.update_layout(title= index[a] +' Comparison between avg corn kg and land cost for district')
    # fig1.update_yaxes(title='land cost')
    # fig1.update_xaxes(title='avg corn kg')
    fig1.write_html(index[a] +' Comparison between avg corn kg and land cost for district.html')
    fig1.show()

    fig2 = px.scatter(df_yield_corn, x = 'land_cost' , y = 'land_limits' , color = 'State', size = 'soy kg')
    fig2.update_layout(title=index[a] +' Comparison between avg soy kg and land cost for district')
    # fig2.update_yaxes(title='land cost')
    # fig2.update_xaxes(title='avg soy kg')
    fig2.write_html(index[a] +" Comparison between avg soy kg and land cost for district.html")
    fig2.show()
    
    fig3 = px.scatter(df_yield_corn, x = 'land_cost' , y = 'land_limits' , color = 'State', size = 'grass kg')
    fig3.update_layout(title=index[a] +' Comparison between avg grass kg and land cost for district')
    # fig3.update_yaxes(title='land cost')
    # fig3.update_xaxes(title='avg grass kg')
    fig3.write_html(index[a] +" Comparison between avg grass kg and land cost for district.html")
    fig3.show()
    
    fig4 = px.scatter(df_yield_corn, x = 'land_cost' , y = 'land_limits' , color = 'State', size = 'algae kg')
    fig4.update_layout(title=index[a] +' Comparison between avg algae kg and land cost for district')
    # fig4.update_yaxes(title='land cost')
    # fig4.update_xaxes(title='avg algae kg')
    fig4.write_html(index[a] +" Comparison between avg algae kg and land cost for district.html")
    fig4.show()
