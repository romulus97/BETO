...
min_10_cost = sorting_cost.tail(1)
...
CG_ethanol_total_MJ = np.zeros((len(LC),len(years)))
SB_oil_total_MJ = np.zeros((len(LC),len(years)))
CS_total_MJ = np.zeros((len(LC),len(years)))
G_energy_total_MJ = np.zeros((len(LC),len(years)))
A_energy_total_MJ = np.zeros((len(LC),len(years)))
...
for ind in ind_min_cost:
    ...
    for year in years:
        ...
        CG_ethanol = CG_processing.sim(CG_kg_tot[:,i]) #kg ethanol
        CG_ethanol_total_MJ[:,i] = CG_ethanol*29.7   #MJ/kg
        
    
        SB_oil = SB_processing.sim(SB_kg_tot[:,i])  #kg soy oil
        SB_oil_total_MJ[:,i] = SB_oil*39.6           #MJ/kg
        
        CS_total_MJ[:,i] = CG_ethanol*29.7 + SB_oil*39.6   #MJ
        
        G_energy = G_processing.sim(G_kg_tot[:,i]) # kg biocrude 
        G_energy_total_MJ[:,i] = G_energy*21         #MJ
        
        A_energy = A_processing.sim(A_kg_tot[:,i]) # kg algae oil
        A_energy_total_MJ[:,i] = A_energy*22         #MJ
    ...
    dist_energy_c = np.zeros((len(district))) # creating empty list for 106 mean yield data set
    dist_energy_s = np.zeros((len(district))) # creating empty list for 106 mean yield data set
    dist_energy_g = np.zeros((len(district))) # creating empty list for 106 mean yield data set
    dist_energy_a = np.zeros((len(district))) # creating empty list for 106 mean yield data set
    ...
    for dist in district:
        ...
        CE = sum(CG_ethanol_total_MJ[i,:]) # for each district summation of total yield throughout 63 years founded
        # total_energy_c[i] = CE # set all total yield value into created list 
        mean_energy = CE/(len(years)) #calculating mean yield for each district by dividing year length
        dist_energy_c[i] = mean_energy  # set all mean yield value into created list 
    ...
    for dist in district:
        ...
        SE = sum(SB_oil_total_MJ[i,:]) # for each district summation of total yield throughout 63 years founded
        # total_energy_c[i] = CE # set all total yield value into created list 
        mean_energy = SE/(len(years)) #calculating mean yield for each district by dividing year length
        dist_energy_s[i] = mean_energy  # set all mean yield value into created list
    ...
    for dist in district:
        ...
        GE = sum(G_energy_total_MJ[i,:]) # for each district summation of total yield throughout 63 years founded
        # total_energy_c[i] = CE # set all total yield value into created list 
        mean_energy = GE/(len(years)) #calculating mean yield for each district by dividing year length
        dist_energy_g[i] = mean_energy  # set all mean yield value into created list
    ...
    for dist in district:
        ...
        AE = sum(A_energy_total_MJ[i,:]) # for each district summation of total yield throughout 63 years founded
        # total_energy_c[i] = CE # set all total yield value into created list 
        mean_energy = AE/(len(years)) #calculating mean yield for each district by dividing year length
        dist_energy_a[i] = mean_energy  # set all mean yield value into created list
    ...
    df_yield_corn['corn_electricity_cost'] = CG_el/dist_energy_c
    df_yield_corn['corn_electricity_cost'] = df_yield_corn['corn_electricity_cost'].fillna(0)
    df_yield_corn['soy_electricity_cost'] = SB_el/dist_energy_s
    df_yield_corn['soy_electricity_cost'] = df_yield_corn['soy_electricity_cost'].fillna(0)
    df_yield_corn['grass_electricity_cost'] = G_el/dist_energy_g
    df_yield_corn['grass_electricity_cost'] = df_yield_corn['grass_electricity_cost'].fillna(0)
    df_yield_corn['algae_electricity_cost'] = A_el/dist_energy_a
    df_yield_corn['algae_electricity_cost'] = df_yield_corn['algae_electricity_cost'].fillna(0)
    ...
    fig1.write_html(index[a] +' Comparison between avg corn kg and land cost vs electricity cost for district_den.html')
    fig1.show()
    ...
    # fig2.write_html(index[a] +" Comparison between avg soy kg and land cost vs electricity cost for district_den.html")
    # fig2.show()
    ...
    # fig3.write_html(index[a] +" Comparison between avg grass kg and land cost vs electricity cost for district_den.html")
    # fig3.show()
    ...
    # fig4.write_html(index[a] +" Comparison between avg algae kg and land cost vs electricity cost for district_den.html")
    # fig4.show()
