...
df_geo_algea = pd.read_excel('combined_pivot_algea_excel_electricity.xlsx',header=0, engine='openpyxl') #contains every eg_district code  
...
cost_algal = list(df_geo_algea['algea_cost_per_ha'])
cost_algal_kg = list(df_geo_algea['algea_cost_per_kg'])
...
def simulate(
    ...
    vg = (np.array(vars[num_c:2*num_c])) #they should be seperate with algea and grass
    va = (np.array(vars[num_c*2:]))
    ...
    for year in years:
        ...
        A_energy = A_processing.sim(A_prod) #L
        A_energy_total[i] = A_energy
        
        Energy =(29.7 * CG_ethanol + 39.6 * SB_oil + G_energy + 174.74 * A_energy)   # Ethanol - 29.7 MJ/kg and Soy Oil - 39.6 MJ/kg
    ...
    Constraints.append(Q * 0.98 - np.mean(Energy_total)) #LB   ##widen that 
    ...
    G_cultivation_opex += np.sum((va[0:num_c]*(A_cost_per_ha))*len(years) +(A_prod_total*(A_cost_per_kg)))
    ...
    biomass_cost_algal = A_cultivation_capex + A_cultivation_opex
