...
def QD (districts,locations):
    ...
    df_geo_a = pd.read_excel('combined_pivot_algea_excel_electricity.xlsx',header=0, engine='openpyxl') #contains every eg_district code  
    ...
    C_yield = df_geo.loc[:,1998:].values
    S_yield = df_geo_s.loc[:,1998:].values
    G_yield = df_geo_g.loc[:,1998:].values
    A_yield = df_geo_a.loc[:,1998:].values  
    ########################################################################
    #########        PRE-PROCESSING       ##################################
    ########################################################################
    ...
    for b in years:
        ...
        for c in range(0,len(LC)):
            ...
            G_D2H_prod[c,:] = D2H_map[c,:]*G_Y[c]*(V1[c])  # should I write V1[c]/2 or V1[c]. V1[c] feels correct to me 
        ...
        Energy_corn = ( 29.7 * CG_ethanol + 39.6 * SB_oil + G_energy + 174.74 * A_energy)   # Ethanol - 29.7 MJ/kg and Soy Oil - 39.6 MJ/kg - dry algae biomass is 174.74 MJ/kg 
