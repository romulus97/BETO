...
def test(groups,num_counties):
    ...
    V = []
    for i in range(0,len(reduced_counties)):
        V.append(reduced_land_limits[i]) #50% of theoretical capacity
    ...
    #??? - Why *(len(hubs)) at the end?
    
    CS_C2H_prod = c2h_map * np.transpose(np.array([(CS_per_ha * V[0:len(LC)]),])) #* len(hubs)))
    ...
    for j in range(0,len(hubs)):
        ...
        for k in range(0,len(locations)):
            ...
            #Spreads out hub biomass to every location
