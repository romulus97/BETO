...
def QD (districts,locations):
    ...
    df_geo_g_L = pd.read_excel('combined_pivot_land_grass_excel_electricity.xlsx',header=0, engine='openpyxl') #contains every eg_district code
    ...
    df_geo_a_L = pd.read_excel('combined_pivot_land_algae_excel_electricity.xlsx',header=0, engine='openpyxl') #contains every eg_district code
    ...
    marginal_land_limits_g = df_geo_g['land_limits_ha'].values # county ag production area in acres
    marginal_land_limits_a = df_geo_a['land_limits_ha'].values # county ag production area in acres
    ...
    G_yield_L = df_geo_g_L.loc[:,1998:2013].values
    G_yield = df_geo_g.loc[:,1998:2013].values
    A_yield_L = df_geo_a_L.loc[:,1998:2013].values  
    ...
    LC = land_limits # land costs per county
    
    V1 = []
    for a in range(0,len(districts)):
        V1.append((marginal_land_limits_g[a])) 
    
    V2 = []
    for a in range(0,len(districts)):
        V2.append((marginal_land_limits_a[a])) 
    ...
    for b in years:
        ...
        GL_flow_matrix = np.zeros((len(hubs),len(locations)))
        GL_D2H_prod = np.zeros((len(LC),len(hubs)))
        GL_refinery_kg = 0
        GL_energy = np.zeros((len(locations),1))
        ...
        AL_flow_matrix = np.zeros((len(hubs),len(locations)))
        AL_D2H_prod = np.zeros((len(LC),len(hubs)))
        AL_refinery_kg = 0
        AL_energy = np.zeros((len(locations),1))
        ...
        for c in range(0,len(LC)):
            ...
            # GL_D2H_prod[c,:] = D2H_map[c,:]*G_Y[c]*(V[c])
            G_D2H_prod[c,:] = D2H_map[c,:]*G_Y[c]*(V1[c])  
            AL_D2H_prod[c,:] = D2H_map[c,:]*A_Y_L[c]*(V[c])
            A_D2H_prod[c,:] = D2H_map[c,:]*A_Y[c]*(V2[c])
        ...
        for d in range(0,len(hubs)):
            ...
            for e in range(0,len(locations)):
                ...
                H2H_A_flow_L.append(sum(AL_D2H_prod[:,d])*(1/len(locations)))
            
                # Mass transfer (kg of CS) from hub 'j' to hub 'k'
                AL_flow_matrix[d,e] = AL_flow_matrix[d,e] + H2H_A_flow_L[d*len(locations) + e]
        ...
        for k in range(0,len(locations)):
            ...
            # # Soy oil produced at refinery at hub 'l'
            # SB_oil[k] = SB_processing.sim(SB_refinery_kg)
            
            # Find total mass received at hub 'l'
            G_refinery_kg = sum(G_flow_matrix[:,k])
            
            # Biocrude produced at refinery at hub 'l'
            G_energy[k] = G_processing.sim(G_refinery_kg)
            
            # Find total mass received at hub 'l'
            AL_refinery_kg = sum(AL_flow_matrix[:,k])
            
            # Algal oil produced at refinery at hub 'l'
            AL_energy[k] = A_processing.sim(AL_refinery_kg)
        ...
        Energy_corn = (21 * G_energy + 22 * AL_energy + 22 * A_energy) 
        
        s = sum(Energy_corn[:])
        # Q.append(s[0])
        Q_series.append(s[0]) #total MJ production
        Q_min = min(Q_series)
