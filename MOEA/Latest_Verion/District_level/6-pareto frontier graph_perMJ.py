...
for b in billion:
    ...
    for v in version:
        ...
        for year in years:
            ...
            G_L = G_yield[:,i]   # grass yield kg/ha
            A_L = A_yield[:,i]   # algae yield kg/ha
            ...
            for s in solution:
                CG_dd = sum(CG_kg_tot_den[:,s])/len(years)
                CG_kg_tot_dd[:,s] = CG_dd     # average corn biomass production for 15 years (kg)
                
                SB_dd = sum(SB_kg_tot_den[:,s])/len(years)
                SB_kg_tot_dd[:,s] = SB_dd       # average soy biomass production for 15 years (kg)
                
                G_dd = sum(G_kg_tot_den[:,s])/len(years)
                G_kg_tot_dd[:,s] = G_dd        # average grass biomass production for 15 years (kg)
                
                A_dd = sum(A_kg_tot_den[:,s])/len(years)
                A_kg_tot_dd[:,s] = A_dd       # average algae biomass production for 15 years (kg)
                
                G_dd_L = sum(G_kg_tot_den_L[:,s])/len(years)
                G_kg_tot_dd_L[:,s] = G_dd_L        # average grass biomass production for 15 years (kg)
                
                A_dd_L = sum(A_kg_tot_den_L[:,s])/len(years)
                A_kg_tot_dd_L[:,s] = A_dd_L       # average algae biomass production for 15 years (kg)    
        ...
        for s in solution:         
            ...
            G_tL = G_total_L[s]   # grass yield kg/ha
            A_tL = A_total_L[s]   # algae yield kg/ha
            ...
            G_energy_L = G_processing.sim(G_tL) # kg biocrude 
            G_energy_total_L[s] = G_energy_L
            
            A_energy_L = A_processing.sim(A_tL) # kg algae oil
            A_energy_total_L[s] = A_energy_L
            
            Energy =(29.7 * CG_ethanol + 36.10 * SB_oil + 15.66* G_energy + 15.66 * A_energy+ 41.65* G_energy_L + 41.65 * A_energy_L)   # Ethanol * 29.7 MJ/kg + Soy Oil * 39.6 MJ/kg + biocrude * 21 MJ/kg + algae oil * 22 MJ/kg
