...
def simulate(
    ...
    f_min_MSFP = min_MSFP/0.02035
    min_shortfall = max(Z)
    f_min_shortfall = min_shortfall/Q
    min_GHG = sum(GHG)/sum(Energy_f)
    f_min_GHG = min_GHG/29.1159

    return [f_min_MSFP, f_min_shortfall, f_min_GHG], Constraints # $/MJ , MJ, gco2/MJ, MJ
...
# What algorithm?
algorithm = [(NSGAIII, {"divisions_outer":12})]

# Evaluate function # of times
results = experiment(algorithm, problem, nfe=1000)
...
df_D = pd.DataFrame(D)
fn = 'Decision_Variables_borg_crops_GHG3' + version + '.csv'
df_D.to_csv(fn)

df_O = pd.DataFrame(O)
fn2 = 'Objective_functions_borg_crops_GHG3' + version + '.csv'
df_O.to_csv(fn2)
