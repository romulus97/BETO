...
# Corn Stover
# C_yield = df_yield['Yield_for_model']  #yield in bushels per acre
...
p=.25
quota, UB , v_test = multiyear_quota.QD(districts,locations,p)
quota = quota[0]
...
def simulate(
    ...
    return [np.sum(v[0:num_c]), np.sum(CS_refinery_capex), CS_travel_opex], Constraints
