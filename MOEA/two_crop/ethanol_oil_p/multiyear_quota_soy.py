# -*- coding: utf-8 -*-
"""
Created on Thu Oct 28 11:47:53 2021
...
def QD (districts,locations):
    ...
    df_geo = pd.read_excel('combined_pivot_soy_excel_electricity.xlsx',header=0, engine='openpyxl') #contains every eg_district code 
    ...
    for b in years:
        ...
        for c in range(0,len(LC)):
            ...
            SB_D2H_prod[c,:] = D2H_map[c,:]*S_Y[c]*V[c]
        ...
        for d in range(0,len(hubs)):
            ...
            for e in range(0,len(locations)):
                ...
                H2H_flow.append(sum(SB_D2H_prod[:,d])*(1/len(locations)))
            
                # Mass transfer (kg of CS) from hub 'j' to hub 'k'
                SB_flow_matrix[d,e] = SB_flow_matrix[d,e] + H2H_flow[d*len(locations) + e]
        ...
        s = sum(SB_oil[:])
        # Q.append(s[0])
        Q_S_series.append(s[0]) #total corn grain kg 
        Q_S_min = min(Q_S_series)

    return Q_S_min,Q_S_series 
